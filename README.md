...
- The action resolves the Coder user to run as. Either pass `github-user-id` (the action looks up the Coder user with that linked GitHub identity) or pass `coder-username` directly. The two inputs are mutually exclusive.
  ...
  - Soft-deleted Coder users are filtered out of the GitHub-id lookup. The action sends `status:active` alongside the `github_com_user_id:` filter and additionally drops any rows tagged `deleted: true` in the response, so a previously deleted account that retained its `github_com_user_id` no longer makes the lookup ambiguous (closes [coder/create-task-action#8](https://github.com/coder/create-task-action/issues/8) for this action; the bug stays open in `create-task-action`).
